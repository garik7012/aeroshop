...
@section('content')
    ...
    <section class="content-header">
        ...
        @include('admin._partials.breadcrumbs', $breadcrumbs = [
            ...
            ['name' => 'Редактирование', 'active' => true]
        ])
    </section>
    <!-- Main content -->
            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">Редактирование статьи {{$article->title}}</h3>
                    </div>
                    <!-- /.box-header -->
                    <!-- form start -->
                    <form role="form" action="{{route('admin.articles.update', $article->id)}}" method="post" enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="title" class="form-control" id="InputName" value="{{old('title') ?: $article->title}}">
                                @if ($errors->has('title'))
                            ...
                            <div class="form-group {{ $errors->has('description') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="description" class="form-control" id="InputDescription" value="{{old('description') ?: $article->description}}">
                            ...
                            <div class="form-group{{ $errors->has('image') ? ' has-error' : '' }}">
                                <label for="exampleInputFile">Заглавное изображение статьи
                                    ...
                                    <a href="{{asset($article->image)}}" target="_blank"><img src="{{asset($article->image)}}" alt="" width="400"></a>
                                    @if ($errors->has('image'))
                                        <span class="help-block">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="content" id="pageContent" rows="10"
                                          class="form-control turmbowyg">{{old('content') ?: $article->content}}</textarea>
                            </div>
                            <h4>СЕО</h4>
                            <div class="form-group {{ $errors->has('seo_description') ? ' has-error' : '' }}">
                                <label for="InputDescription">СЕО description (описание в гугле)</label>
                                <input type="text" name="seo_description" class="form-control" id="InputDescription" value="{{old('seo_description') ?: $article->seo_description}}">
                            ...
                            <div class="form-group {{ $errors->has('keywords') ? ' has-error' : '' }}">
                                ...
                                <input type="text" name="keywords" class="form-control" id="InputDescription" value="{{old('keywords') ?: $article->keywords}}">
                            ...
                            <div class="checkbox {{ $errors->has('is_active') ? ' has-error' : '' }}">
                                <label>
                                    ...
                                    <input type="checkbox" name="is_active" class="" id="InputIsActive" value="1" {{$article->is_active ? 'checked': ''}}>
